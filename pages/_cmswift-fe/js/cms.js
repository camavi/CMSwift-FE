(() => {
    ...
    for (const arg of args) {
      ...
      if (Array.isArray(arg)) {
        ...
        while (stack.length) {
          ...
          // se ti arriva un oggetto props dentro array per errore, puoi ignorarlo o gestirlo
          // qui lo ignoriamo per evitare side effects
        }
        continue;
      }
  ...
  // ===============================
  // HTTP Plugin (fetch wrapper)
  // ===============================
  CMSwift.plugins.http = {
    install(app, opts = {}) {
      const config = {
      ...
      function normalizeRequest(input, init = {}) {
        const url = typeof input === "string" ? joinURL(config.baseURL, input) : input;
      ...
      async function request(input, init = {}) {
        ...
        // hooks pre
        req = await runHooks(hooks.beforeRequest, req);
        ...
        for (let attempt = 0; attempt <= maxAttempts; attempt++) {
          ...
          try {
            if (config.debug) console.log("[http.request]", effectiveReq.method, effectiveReq.url);
            ...
            // hooks post
            const outRes = await runHooks(hooks.afterResponse, res, effectiveReq);
      ...
      app.http = {
        ...
        onBefore(fn) { hooks.beforeRequest.add(fn); return () => hooks.beforeRequest.delete(fn); },
        onAfter(fn) { hooks.afterResponse.add(fn); return () => hooks.afterResponse.delete(fn); },
        onError(fn) { hooks.onError.add(fn); return () => hooks.onError.delete(fn); },
